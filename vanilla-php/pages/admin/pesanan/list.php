...
$pesanans = $db->fetchAll(
    "SELECT p.*, u.name, u.email FROM pesanans p LEFT JOIN users u ON p.user_id = u.id ORDER BY p.created_at DESC"
);
?>
        ...
        <tbody>
            <?php foreach ($pesanans as $pesanan): ?>
                <tr>
                    ...
                    <td>
                        <a href="/vanilla-php/admin/pesanan/<?php echo $pesanan['id']; ?>" class="btn" style="padding: 0.25rem 0.75rem; font-size: 0.85rem;">Lihat</a>
                    </td>
                </tr>
            <?php endforeach; ?>
        </tbody>
